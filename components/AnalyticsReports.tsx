...
import { Printer, Calendar, Users, Activity, FileSpreadsheet, Clock, ChevronDown, AlertCircle, X, CheckCircle2, MinusCircle, XCircle, Filter, UserCheck, Search } from 'lucide-react';
...
const AnalyticsReports: React.FC<AnalyticsReportsProps> = ({ employees, logs, tasks, assignments }) => {
  ...
  const individualStats = useMemo(() => {
    ...
    const loggedWorkDates = new Set(userLogs.filter(l => l.status !== 'Leave' && l.status !== 'إجازة' && l.status !== 'عطلة').map(l => l.logDate.split('T')[0]));
  ...
  const collectiveStats = useMemo(() => {
    return employees
      ...
      .map(emp => {
        ...
        const loggedWorkDates = new Set(uLogs.filter(l => l.status !== 'Leave' && l.status !== 'إجازة' && l.status !== 'عطلة').map(l => l.logDate.split('T')[0]));
        ...
        const completedCount = uLogs.filter(l => (l.status === 'Completed' || l.status === 'منفذة') && l.taskType === 'Daily').length;
        const pendingCount = uLogs.filter(l => (l.status === 'Pending' || l.status === 'غير منفذة') && l.taskType === 'Daily').length;
        const naCount = uLogs.filter(l => (l.status === 'NotApplicable' || l.status === 'لا تنطبق') && l.taskType === 'Daily').length;
  ...
  const filteredEmployeesForSelection = employees.filter(e => 
    e.name.includes(empSearchTerm) || e.id.includes(empSearchTerm)
  );

  return (
    <div className="space-y-6 animate-fade-in print:p-0">
      {/* شريط الأدوات العلوي المطور */}
              ...
              {viewMode === 'individual' ? (
                ...
                <div className="flex-1 relative">
                  ...
                  <button 
                    ...
                    className="w-full bg-gray-50 border-2 border-gray-100 p-3.5 rounded-2xl font-black text-gray-700 flex items-center justify-between hover:border-indigo-200 transition-all h-[54px] shadow-sm"
                  >
                    <div className="flex items-center gap-2">
                       <Filter size={16} className="text-indigo-400" />
                       <span className="text-xs">
                          {selectedComparisonEmps.length === employees.length ? 'تم اختيار جميع الموظفين' : `تم اختيار ${selectedComparisonEmps.length} موظف`}
                       </span>
                    </div>
                    <ChevronDown size={18} className={`transition-transform duration-300 ${showEmpSelector ? 'rotate-180' : ''}`} />
                  </button>
                  
                  {showEmpSelector && (
                    <div className="absolute top-full right-0 left-0 mt-3 bg-white border border-gray-100 rounded-[32px] shadow-2xl z-[100] p-6 max-h-[450px] overflow-y-auto animate-scale-in">
                       <div className="flex flex-col gap-4 mb-6">
                          <div className="relative">
                             <input 
                               ...
                               className="w-full p-4 pr-12 bg-gray-50 border border-gray-100 rounded-2xl text-xs font-bold outline-none focus:border-indigo-300"
                             />
                             <Search className="absolute right-4 top-4 text-gray-300" size={18} />
                          </div>
                          <div className="flex justify-between items-center px-2">
                             <button onClick={() => setSelectedComparisonEmps(employees.map(e => e.id))} className="text-[10px] font-black text-indigo-600 uppercase hover:underline">تحديد الكل</button>
                             <button onClick={() => setSelectedComparisonEmps([])} className="text-[10px] font-black text-red-500 uppercase hover:underline">إلغاء الكل</button>
                          </div>
                       </div>
                       <div className="grid grid-cols-1 gap-2">
                          {filteredEmployeesForSelection.map(e => (
                            <label key={e.id} className={`flex items-center gap-3 p-4 rounded-2xl cursor-pointer group transition-all border-2 ${selectedComparisonEmps.includes(e.id) ? 'bg-indigo-50/50 border-indigo-200' : 'bg-white border-transparent hover:bg-gray-50'}`}>
                               <input 
                                type="checkbox" 
                               ...
                               />
                               <div className="flex-1">
                                  <p className={`text-sm font-black ${selectedComparisonEmps.includes(e.id) ? 'text-indigo-700' : 'text-gray-600'}`}>{e.name}</p>
                                  <p className="text-[10px] text-gray-400 font-bold">ID: {e.id}</p>
                               </div>
                            </label>
                          ))}
                          {filteredEmployeesForSelection.length === 0 && (
                             <p className="text-center py-4 text-gray-400 text-xs font-bold">لا توجد نتائج بحث</p>
                          )}
                       </div>
                    </div>
                  )}
              ...
              {timePreset === 'custom' && (
                <div className="flex gap-3 items-end animate-scale-in">
                   <div>
                      ...
                      <input type="date" value={dateRange.start} onChange={e=>setDateRange({...dateRange, start: e.target.value})} className="bg-gray-50 border border-gray-100 p-3.5 rounded-2xl font-black text-xs h-[54px] outline-none focus:border-indigo-200 shadow-sm" />
                   </div>
                   <div>
                      <label className="text-[10px] font-black text-gray-400 uppercase block mb-2">إلى تاريخ</label>
                      <input type="date" value={dateRange.end} onChange={e=>setDateRange({...dateRange, end: e.target.value})} className="bg-gray-50 border border-gray-100 p-3.5 rounded-2xl font-black text-xs h-[54px] outline-none focus:border-indigo-200 shadow-sm" />
                   </div>
                </div>
              )}
           </div>
      ...
      <div className="bg-white p-10 md:p-14 rounded-[48px] shadow-sm border border-gray-100 min-h-[900px] print:p-0 print:border-none print:shadow-none">
          ...
          <div className="space-y-12 animate-fade-in">
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="p-10 bg-blue-50/50 rounded-[40px] border border-blue-100 text-center flex flex-col justify-center items-center shadow-sm">
                    ...
                    <div className="p-10 bg-green-50/50 rounded-[40px] border border-green-100 text-center flex flex-col justify-center items-center shadow-sm">
                    ...
                    <div className="p-10 bg-red-50/50 rounded-[40px] border border-red-100 text-center flex flex-col justify-center items-center shadow-sm">
                    ...
                    <div className="p-10 bg-orange-50/50 rounded-[40px] border border-orange-100 text-center flex flex-col justify-center items-center shadow-sm">
            ...
            <div className="space-y-8 pt-8">
               <h3 className="text-2xl font-black text-gray-900 flex items-center gap-3 pr-2">
                 <div className="w-2 h-8 bg-indigo-600 rounded-full"></div> 
                 تحليل أداء جميع المهام المعينة (ترتيب تسلسلي)
               </h3>
               ...
               <div className="space-y-10">
                  {individualStats.taskAnalysis.map(t => (
                    <div key={t.id} className="bg-white p-10 rounded-[48px] border border-gray-100 shadow-sm transition-all hover:shadow-xl hover:translate-y-[-4px] group">
                       <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                          <div className="flex-1">
                             <div className="flex items-center gap-3 mb-3">
                                <span className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center font-black text-sm">{t.id}</span>
                                <h4 className="text-2xl font-black text-gray-800 leading-tight group-hover:text-indigo-900 transition-colors">{t.desc}</h4>
                             </div>
                             <p className="text-[11px] text-gray-400 font-bold uppercase tracking-widest pr-12">نسبة الالتزام بالهدف (التقارير المتوقعة)</p>
                          </div>
                       ...
                       <div className="relative h-10 bg-gray-50 rounded-full overflow-hidden shadow-inner border border-gray-100 p-1.5">
                          <div 
                            className={`h-full transition-all duration-1000 flex items-center justify-end px-6 rounded-full shadow-lg ${t.rate >= 80 ? 'bg-green-500' : t.rate >= 50 ? 'bg-amber-500' : 'bg-red-500'}`} 
                            style={{width: `${t.rate}%`}}
                          >
        ...
        {viewMode === 'collective' && (
          <div className="space-y-16 animate-fade-in">
            ...
            <div className="w-32 h-2 bg-indigo-600 mx-auto rounded-full shadow-lg"></div>
            ...
            <div className="space-y-10">
               ...
               <div className="space-y-8 bg-gray-50/50 p-12 rounded-[60px] border border-gray-100">
                  ...
                  {collectiveStats.length === 0 && (
                    <p className="text-center py-10 text-gray-400 font-black">يرجى اختيار موظفين للمقارنة من شريط الأدوات العلوي</p>
                  )}
               </div>
            </div>
            ...
            <div className="space-y-10">
               ...
               <div className="overflow-x-auto rounded-[48px] border border-gray-100 shadow-2xl overflow-hidden bg-white">
                  <table className="w-full text-right text-sm">
                     <thead className="bg-gray-50 text-gray-400 font-black uppercase tracking-widest border-b border-gray-100">
                        <tr>
                           ...
                           <th className="p-10 text-center text-green-600">منفذة (كعدد)</th>
                     ...
                     <tbody className="divide-y divide-gray-50">
                        {collectiveStats.map(s => (
                           <tr key={s.id} className="hover:bg-indigo-50/30 transition-all duration-300">
                              <td className="p-10">
                                 <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 bg-gray-100 rounded-xl flex items-center justify-center font-black text-gray-400 shadow-sm">{s.name.charAt(0)}</div>
                              ...
                              <td className="p-10 text-center">
                                 ...
                                 <p className="text-[10px] text-gray-400 font-bold">{s.actual} من {s.expected} يوم</p>
                              </td>
